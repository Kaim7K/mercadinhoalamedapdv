...
export default function PDV() {
  ...
  const loadProducts = async () => {
    ...
    try {
      const data = await appClient.entities.Product.list('-updated_date', 500);
  ...
  const loadMaxMinimized = async () => {
    try {
      const configs = await appClient.entities.SystemConfig.filter({ key: 'limite_vendas_minimizadas' });
  ...
  const getNextSaleNumber = async () => {
    try {
      const sales = await appClient.entities.Sale.list('-sale_number', 1);
  ...
  const handlePriceCorrection = async (idx, newPrice) => {
    ...
    await appClient.entities.Product.update(product.id, { sale_price: newPrice });
    ...
    await appClient.entities.ProductAudit.create({ product_id: product.id, product_name: product.name, field_changed: 'sale_price', previous_value: String(oldPrice), new_value: String(newPrice), user_id: user.id, user_name: user.full_name || user.email, change_origin: 'valor_errado', sale_number: saleNumber, observation: 'Correção de valor na tela de vendas' });
    await appClient.entities.GeneralAudit.create({ action_type: 'alteracao_preco', entity_type: 'product', entity_id: product.id, user_id: user.id, user_name: user.full_name || user.email, description: `Preço de ${product.name} alterado de ${formatCurrency(oldPrice)} para ${formatCurrency(newPrice)}`, details: JSON.stringify({ old: oldPrice, new: newPrice, sale_number: saleNumber }) });
  ...
  const completeSale = async (paymentData) => {
    try {
      ...
      const sale = await appClient.entities.Sale.create({
      ...
      for (const item of activeSale.items) {
        ...
        if (product) {
          ...
          await appClient.entities.Product.update(product.id, { quantity: (product.quantity || 0) - qtyChange });
        }
      }
      if (paymentData.sale_type === 'fiado' && paymentData.fiado) {
        await appClient.entities.FiadoRecord.create({
      ...
      await appClient.entities.GeneralAudit.create({ action_type: 'venda_concluida', entity_type: 'sale', entity_id: sale.id, user_id: user.id, user_name: user.full_name || user.email, description: `Venda #${sale.sale_number} concluída - ${formatCurrency(total)}`, details: JSON.stringify({ total, items: activeSale.items.length, sale_type: paymentData.sale_type }) });
