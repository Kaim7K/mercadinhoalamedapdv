...
export default function Configuracoes() {
  ...
  const loadConfigs = async () => {
    try {
      const data = await appClient.entities.SystemConfig.list();
      const map = {};
  ...
  const handleSave = async () => {
    ...
    try {
      for (const key of Object.keys(configs)) {
        ...
        if (cfg.id) {
          await appClient.entities.SystemConfig.update(cfg.id, { value: cfg.value });
        } else {
          await appClient.entities.SystemConfig.create({ key: cfg.key, value: cfg.value, label: cfg.label || cfg.key });
        }
      }
