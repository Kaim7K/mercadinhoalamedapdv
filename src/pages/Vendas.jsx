...
export default function Vendas() {
  ...
  const loadSales = async () => {
    setLoading(true);
    try {
      let data = await appClient.entities.Sale.list('-created_date', 200);
  ...
  const handleCancel = async (sale) => {
    ...
    try {
      await appClient.entities.Sale.update(sale.id, { status: 'cancelada', cancellation_reason: reason, cancelled_by_id: user.id, cancelled_by_name: user.full_name || user.email });
      await appClient.entities.GeneralAudit.create({ action_type: 'venda_cancelada', entity_type: 'sale', entity_id: sale.id, user_id: user.id, user_name: user.full_name || user.email, description: `Venda #${sale.sale_number} cancelada`, details: JSON.stringify({ reason, total: sale.total }) });
  ...
  const handleDelete = async (sale) => {
    ...
    try {
      await appClient.entities.Sale.delete(sale.id);
      await appClient.entities.GeneralAudit.create({ action_type: 'venda_excluida', entity_type: 'sale', entity_id: sale.id, user_id: user.id, user_name: user.full_name || user.email, description: `Venda #${sale.sale_number} excluída`, details: JSON.stringify({ total: sale.total, items: sale.items?.length }) });
